"""
DEFINITION FILE OF NAVIGATION ENVIRONMENTS (W/ TURN-AND-FORWARD DYNAMICS)
"""
...
class RandDistShift3(MiniGridEnv):
    ...
    def __init__(self, width=8, height=8, lava_density_range=[0.3, 0.4], min_num_route=1, transposed=False, gamma=0.99,
                 max_jump_distance=2, random_color=False):
        ...
        super().__init__(width=width, height=height, max_steps=4 * width * height, see_through_walls=True,
                         agent_view_size=15)  # Set this to True for maximum speed
        ...
        print('Observation Init:', self.obs_curr)
    ...
    def reset(self):
        super().reset()
        self.obs_curr = self.gen_fullyobservable_obs()
        print('Observation on Reset:', self.obs_curr)
    ...
    def step(self, action):
        ...
        self.obs_curr = self.gen_fullyobservable_obs()
        print(f'Observation on Action {action}:', self.obs_curr)
        return self.obs_curr, np.sign(reward), done, {}


environment = RandDistShift3()
# environment.reset()
test = environment.attention_render()
plt.imsave('test.png', test.reshape((256, 256, 3)))
